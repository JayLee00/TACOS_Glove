...
class SaveTactile:
    ...
    def __init__(self, tactile:Tactile, kalman_en = True, save_dir=None):
        self.tactile = tactile
        self.kalman_en = kalman_en

        # 저장할 기본 디렉토리를 설정합니다.
        # 만약 save_dir가 지정되지 않으면, 현재 작업 디렉토리 아래의 'SAVEFILES' 폴더를 기본값으로 사용합니다.
        if save_dir is None:
            # 이 스크립트가 있는 'ljs' 폴더를 기준으로 'SAVEFILES' 폴더 경로를 만듭니다.
            # __file__은 현재 파일의 경로입니다. (e.g., .../ljs/SaveLoad/save.py)
            ljs_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            self.save_dir = os.path.join(ljs_dir, "SAVEFILES")
        else:
    ...
    def save(self, filename = "timestamp_data"):#묵시적
       ...
       # path = f"{os.getcwd()}/0_pc_receiver/kyc/SAVEFILES/{self.filename}.npz"
        path = os.path.join(self.save_dir, f"{self.filename}.npz")
        if self.kalman_en == True:
            np.savez(f"{path}", 
    ...
    def save_lstsq(self, a, b, file_explain = "least_square_data"):
        ...
        path = os.path.join(self.save_dir, f"{self.filename_lstsq}.npz")
    ...
    def save_calib(self, filename_begin = "timestamp_data"):#묵시적
        ...
        path = os.path.join(self.save_dir, f"{filename}.npz")
        np.savez(f"{path}", 
                 tactile_time = self.tactile_time,
